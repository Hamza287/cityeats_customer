...
class CustomDatePickerView extends StatefulWidget {
  const CustomDatePickerView({Key? key}) : super(key: key);

  @override
  State<CustomDatePickerView> createState() => _CustomDatePickerViewState();
}
...
class _CustomDatePickerViewState extends State<CustomDatePickerView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            DateBoxList(dates: generateWeeklyDates(weeklyDates, chosenDate)),
            // TimeRanges(
            //   timeRange: model.isTodaysDateSelected(date: DateTime.now())
            //       ? model.todayTimeRanges
            //       : model.generalTimeRange,
            // ),
          ],
        ),
      ),
  ...
  generateWeeklyDates(List<CustomDate> weeklyDates, DateTime? chosenDate) {
    for (int i = 0; i < 7; i++) {
      weeklyDates.add(
        CustomDate(
...
class TimeRanges extends StatelessWidget {
  ...
  final List<CustomTime> timeRange;
  ...
  Widget build(BuildContext context) {
    return Consumer<RestaurantDetailViewModel>(
      builder: (context, model, child) => GestureDetector(
        onTap: () {
          // model.assignTime(customTime.timeRange);
          // model.selectTime(index);
        },
        child: Container(
          width: 337.w,
